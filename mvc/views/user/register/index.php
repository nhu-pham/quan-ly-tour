        ...
        <div class="login-box" id="loginsection">
            ...
            <form action="register" method="post">
                <div class="input-group">
                    ...
                    <input name="fullname" type="text" placeholder="Nhập họ và tên" id="username" value="<?php
                                echo !empty($data['fullname']) ? $data['fullname'] : '';
                                ?>" required>
                    <p class="error-text"><?= !empty($errors['fullname']) ? $errors['fullname'] : ''; ?></p>
                </div>
                <div class="input-group">
                    <label class="title-login" for="phone_number">Số điện thoại <label
                            class="required">(*)</label></label>
                    <input name="phone_number" type="tel" pattern="[0]{1}[0-9]{9}" placeholder="Nhập số điện thoại"
                ...
                <div class="input-group">
                    ...
                    <input name="email" type="text" placeholder="Nhập email" id="username" value="<?php
                ...
                <div class="input-group">
                    <label class="title-login" for="password">Nhập lại mật khẩu <label
                            class="required">(*)</label></label>
                    <div class="change-psw">
                        <input name="retype-password" type="password" id="confirm-password"
                            placeholder="Nhập lại mật khẩu" required>
                        <button type="button" onclick="togglePassword('confirm-password', this)">
                            <i class="fa-regular fa-eye-slash"></i>
                        </button>
                    </div>
                    <p class="error-text"><?= !empty($errors['retype-password']) ? $errors['retype-password'] : ''; ?>
                    </p>
                </div>
                <button type="submit" id="login-btn" class="signup-btn" onclick="showPopup()">Đăng ký</button>
                <div class="options dk">
                    <label for="">Bạn đã có tài khoản?</label><a href="login"
                        style="color: var(--bright-navy-blue); text-decoration: underline;">Đăng nhập</a>
                </div>
            </form>
        </div>
    </div>
